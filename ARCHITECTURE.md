...
TECH_PLAN.md remains authoritative on **construction** and CONCEPT.md on
**intent**. This file records the **choices** — what was picked, what was
...
**Rejected.** A client-side fetch of `/api/daily` on load. It costs a round trip
before the first paint and makes the instrument wrong for that whole window.
...
submit. Checks now declare `required_deps`, and a level this process cannot run
is dropped at boot with a warning — and scheduling one is a hard failure.
...
marked `dev: true`, never scheduled, and refused in production — because it is
human prose that already reads below the notch, so publishing it as a daily
would be a lie rather than a shortcut. `forge pack` / `forge publish` are the
supported path.
