...
**Pitch:** A craft game. You're given real watermarked text and you launder the watermark out — dropping the detector below its threshold in as few edits as you can. Low floor (anyone can rewrite their way out), high ceiling (the surgical few-word kill tops the leaderboard).
...
- **Par:** the move space is unbounded, so minimum edit distance can't be computed. Set par from an authored reference solution per daily, or from the best clear in the first N plays (self-balancing).
...
One sentence, the needle, and a **length slider**. Drag right → more of a pre-written watermarked passage is revealed → the needle climbs from "could be human" into "detected." Teaches *longer = more evidence* with zero copy. Then a **≥50-word floor** snaps in ("you can't just delete your essay"), killing the obvious "make it shorter" move and motivating rewriting. Hand off to L1.

## Levels (the gate is a checklist; each level adds one check)

| # | Level | Adds to gate | Teaches |
|---|-------|-------------|---------|
| 1 | Clean it | natural + meaning | the loop, the ripple |
| 2 | Budget | ≤ N words changed | placement — cut upstream of a hot run |
| 3 | Locked | a phrase must appear verbatim | launder *around* fixed signal ("quote your source") |
| 4 | Tight fence | meaning clamps to close-paraphrase-only | surgical, faithful edits; cheap rewrites stop working |
| 5 | Code | unit test must pass | why code is a terrible host (almost no entropy to hide in) |
| 6 | Regenerate locally | — | long high-entropy wall; a "regenerate locally" option clears it instantly → the one robust attack: **control the sampler** |

## Daily & share

One **daily passage** for everyone with a shown par. Share headlines **words changed** ("Launder #212 — cleared in 4 🧼"); the brag is the **diff itself** — the 4-word change that killed a 90% detection.

## UX principles (neal.fun-simple)

One screen throughout. No accounts (localStorage streak). Instant play. Pre-filled box — no blank-page freeze. Live feedback is free (local detector); the only paid call is one cheap gate per submit.

## Scope

**v1:** raw textarea; live needle + heat-mirror + ripple; gate (natural + meaning) on submit; edit-distance scoring; daily + par; the 6 levels; slider intro.
...
- **Passage authoring is the real work:** each daily must be solvable-but-not-trivial with a genuine par. Budget for it; underestimating this sinks the game.
