...
That copy is already one of the two `find_repo_root` markers, so the fixture
only has to stub the other one, `pyproject.toml`. Tests that mutate artifacts
(corrupting a manifest digest, deleting a copy blurb) work there so they can
never damage the real checkout — which matters more than usual here, because
several of those artifacts are key material.
"""
